...
v_pdp_pvalue_right <- function(k1_vec, k2_vec, n1, n2, n_samples=9999) {
  ...
  for (i in 1:length(k1_vec)) {
    ...
    p <- permtest_difference_in_props(k1_vec[i],k2_vec[i],n1,n2,n_samples)
...
#              permutations
#
permutation_test <- function(d, var_to_permute, statistic, n_samples=9999,grouping_var) {
  observed_statistic <- statistic(d,var_to_permute,grouping_var)
  permutation_statistics <- rep(0, n_samples)
  for (i in 1:n_samples) {
    d_perm <- randomize(d, var_to_permute)
    permutation_statistics[i] <- statistic(d_perm,var_to_permute,grouping_var)
  }
  result <- list(observed=observed_statistic,
                 permuted=permutation_statistics)
  return(result)
